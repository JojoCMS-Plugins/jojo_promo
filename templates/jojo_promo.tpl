...
<figure>{if $promo.link}<a href="{$promo.link}" title="{$promo.title}">{/if}<img src="{$SITEURL}/images/{$promoimages}/{$promo.image}" alt="" class="img-circle">{if $promo.link}</a>{/if}</figure>{/if}{if $promotitles=='after'}
